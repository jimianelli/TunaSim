DATA_SECTION
  ...
  // add SS3-like logistic maturity schedule (age dependent) 
  // add SS3-like selectivity options
  // add spatial structure (fleet1 <- area1, fleet2 <- area2, ...)
  // add 2 options of the spawning ground setting; spawn at "all area" or "area1 only". 
  int iseed
  int eff_seed
  int len_seed
  int len_dev_seed
  int rec_dev_seed   // OS added
  init_number rho_sd   
  init_number catch_sd   
  init_number sample_size
  init_number effort_sd
  init_int catch_mth // OS temporaly added 
  init_int catch_wek // OS temporaly added
  init_int nyrs  
  init_int nages    
  init_vector mage(1,2)  // OS added as the mim maturity age
  init_int num_fish      // OS commented: number of fisheries (num_fish) equals the number of area(=3). 
  init_int nlint;
  init_matrix effort(1,num_fish,1,nyrs) // num_fish=3
  init_matrix qchange(1,num_fish,1,nyrs) // num_fish=3
  matrix obs_effort(1,num_fish,1,nyrs) // num_fish=3
  init_vector M(1,nages)
  init_vector vbcoff(1,3)
  init_vector sd(1,nages)
  init_vector mean_length_sd(1,nages)
  init_matrix pop1(1,num_fish,1,nages)    // OS changed from "init_vector pop1(1,nages)"  // num_fish=3
  init_int recruit_sw    // OS add: recruitment input switch
  init_matrix init_recruit(1,num_fish,2,nyrs)  // OS changed from "init_vector init_recruit(2,nyrs)"
  ...
  init_vector sel_sw(1,num_fish) // OS add: sel input switch // num_fish=3
  init_matrix selpl(1,num_fish,1,8)  // OS add // num_fish=3
  init_int num_move   // OS add: num_move = num_fish*num_fish =3*3=9
  init_matrix movement(1,num_move,1,nages)  // ex)num_move=9: 1-1,1-2,1-3,2-1,2-2,2-3,3-1,3-2,3-3; sum (of movement rate from each start area)should be 1.0
  init_int spawn_area_sw  // spawning area option; 0= all area, 1= spawn and recruit at last area.
  init_vector q(1,num_fish) // num_fish=3
 ...
 !! J.fill_seqadd(1,1);
  int l1;
  int ln;
  matrix effort_devs(1,num_fish,1,nyrs)  // num_fish=3
  matrix catch_devs(1,num_fish,1,nyrs)  // num_fish=3
  3darray mean_length_devs(1,num_fish,1,nyrs,1,nages) // num_fish=3
  vector cohort_rho_devs(1-nages,nyrs)
  vector rec_devs(1,nyrs)      // OS add for BH
  4darray q1(1,num_fish,1,nyrs,1,nages,1,nlint) // num_fish=3
  number alpha // OS add for BH
  number beta  // OS add for BH
  number delta 
  //number nlint // OS modified
  matrix selplt(1,num_fish,1,8)  // OS add for sel // num_fish=3
...
//Comment; I have to add the area component for every parameter.
 
PARAMETER_SECTION
  matrix sel(1,num_fish,1,nages) // OS added // num_fish=3
  vector mature(1,nages)       // OS added
  matrix SSB(1,num_fish,1,nyrs)   //change from "vector SSB(1,nyrs)" for the spatial structure // OS add for BH (Mt) // num_fish=3
  matrix rbio(1,num_fish,1,nyrs)  //change from "vector rbio(1,nyrs)" for the spatial structure 
  3darray obs_freq(1,num_fish,1,nyrs,1,nlint)  // num_fish=3
  3darray pred_freq(1,num_fish,1,nyrs,1,nlint)  // num_fish=3
  3darray F(1,num_fish,1,nyrs,1,nages)    // num_fish=3
  3darray Z(1,num_fish,1,nyrs,1,nages)    // num_fish=3  // change from "matrix Z(1,nyrs,1,nages)"  for the spatial structure
  3darray S(1,num_fish,1,nyrs,1,nages)    // num_fish=3  // change from "matrix S(1,nyrs,1,nages)"  for the spatial structure
  3darray N(1,num_fish,1,nyrs,1,nages)    // num_fish=3  // change from "matrix N(1,nyrs,1,nages)"  for the spatial structure
  3darray C(1,num_fish,1,nyrs,1,nages)    // num_fish=3
  3darray Cbio(1,num_fish,1,nyrs,1,nages)  // num_fish=3  // OS added
  3darray P(1,num_fish,1,nyrs,1,nages)    // num_fish=3
  matrix obs_catch(1,num_fish,1,nyrs)  // num_fish=3
  matrix obs_catbio(1,num_fish,1,nyrs)  // num_fish=3  // OS added
  matrix mu(1,nyrs,1,nages)
  vector flselpeak1(1,num_fish);  // num_fish=3
  vector flselpeak2(1,num_fish);  // num_fish=3
...
PRELIMINARY_CALCS_SECTION
  ...
  write_simulated_data_for_MFCL();
  write_simulated_data_for_SS3_multi();
  write_simulated_data_for_SS3_mono();
  exit(1);
...
FUNCTION get_mortality_and_survival_rates
  ...
  for (int fi=1;fi<=num_fish;fi++)
  {
    ...
    //Z(fi)+=F(fi);     // OS change from "Z+=F(fi);" for the spatial structure. 
  
    dvar_matrix TZ=trans(F(fi));   
...
FUNCTION get_numbers_at_age
  ...
  if (recruit_sw==1) {
      for (fi=1;fi<=num_fish;fi++) {
            for (i=2;i<=nyrs;i++) {
                N(fi,i,1)=init_recruit(fi,i);
                } 
            for (j=1;j<=nages;j++) {
                N(fi,1,j)=pop1(fi,j);
                SSB(fi,1) += pop1(fi,j)*mature(j)*pow(mu(1,j),lwcoff(2))*lwcoff(1)/1000;
            }
      }
      for (i=1;i<nyrs;i++) {
            for (fi=1;fi<=num_fish;fi++) {
                for (fj=1;fj<=num_fish;fj++) {
                    mi = fi+((fj-1)*num_fish);
                    for (j=1;j<nages;j++) {
                        N(fi,i+1,j+1)+= N(fj,i,j)*S(fj,i,j)*movement(mi,j);
                    }
                    N(fi,i+1,nages)+= N(fj,i,nages)*S(fj,i,nages)*movement(mi,nages);
               }
            }
            for (fi=1;fi<=num_fish;fi++) {
                for (j=2;j<=nages; j++) {
                    SSB(fi,i+1) +=N(fi,i+1,j)*mature(j)*pow(mu(i+1,j),lwcoff(2))*lwcoff(1)/1000;  // SSB(Mt)
                }
            }
      }
  }
      ...
      for (i=1;i<nyrs;i++) {
          for (fi=1;fi<=num_fish;fi++) {
              for (fj=1;fj<=num_fish;fj++) {
                  ...
                  mi = fi+((fj-1)*num_fish);
                      N(fi,i+1,j+1)+= N(fj,i,j)*S(fj,i,j)*movement(mi,j);
                     // N(fi,i+1,j+1)+= N(fj,i,j)*S(fj,i,j)*movement(fi,j);
                     // N(fi,i+1,j+1)+= N(fj,i,j)*S(fj,i,j);
                  }
                  N(fi,i+1,nages)+= N(fj,i,nages)*S(fj,i,nages)*movement(mi,nages);
                  //N(fi,i+1,nages)+= N(fj,i,nages)*S(fj,i,nages)*movement(fi,nages);
                  //N(fi,i+1,nages)+= N(fj,i,nages)*S(fj,i,nages);
              }
          }      
          for (fi=1;fi<=num_fish;fi++) {
              for (j=2;j<=nages;j++) {
              ...
              if(spawn_area_sw==0) {
                  N(fi,i+1,1)=(alpha*SSB(fi,i+1))/(beta+SSB(fi,i+1))*exp(rec_devs(i)+pow((-0.5)*sigmaR,2))*(1-exp(log(0.5)*SSB(fi,i+1)/(v*B0)));
              }
              if(spawn_area_sw==1) {
                  if(fi==1) { 
                      N(1,i+1,1)=(alpha*SSB(1,i+1))/(beta+SSB(1,i+1))*exp(rec_devs(i)+pow((-0.5)*sigmaR,2))*(1-exp(log(0.5)*SSB(1,i+1)/(v*B0)));
                  }
                  else { 
                      N(fi,i+1,1)=0;
                  }
...
FUNCTION get_catch_at_age
  for (int fi=1;fi<=num_fish;fi++)
    C(fi)=elem_prod(elem_div(F(fi),Z(fi)),elem_prod(1.-S(fi),N(fi)));  // OS modified
...
FUNCTION get_biomass_levels
  for (int fi=1;fi<=num_fish;fi++) {
      for (int i=1;i<=nyrs;i++)
          rbio(fi,i)=N(fi,i)*pow(mu(i),lwcoff(2))*lwcoff(1); //L(cm)-W(kg) relationsip like SBT (OS modify)
  }

FUNCTION write_maundered_data
  int i,fi;
  int nregions=num_fish;
...
FUNCTION write_simulated_data_for_MFCL
  int i,j,fi,fj;
  ...
  if(num_fish>1) {
    ...
    for (fi=1;fi<=num_fish-1;fi++) {
      for (fj=1;fj<=num_fish-1;fj++) {
          if(fj < num_fish-1)
            ofs << "  1";
          else 
            ofs << "  1" << endl;
...
FUNCTION write_simulated_data
  ...
  dvar_matrix relbio(1,num_fish,1,nyrs);
  for (fi=1;fi<=num_fish;fi++) { 
      for (i=1;i<=nyrs;i++) {
         relbio(fi,i)= rbio(fi,i)/max(rbio(fi)); 
      }
  }
  ofstream ofs1("tN.dat");
  ofstream ofs2("tB.dat");
  ofstream ofs3("tF.dat");
  ofstream ofs4("tZ.dat");
  ofstream ofs5("trbiom.dat");
  ofstream ofs6("ttot.dat");
  //ofstream ofs95("tinit.dat");
  //ofstream ofs96("tend.dat");
  ...
  for (fi=1;fi<=num_fish;fi++) { 
      for (i=1;i<=nyrs;i++) {
          ofs1 << fi << " "<< i << " " <<   N(fi,i)   << endl;
          ofs2 << fi << " "<< i << " " <<   "SSB(t): " << SSB(fi,i) << "   Biomass(kg):  " << rbio(fi,i)  << endl;
          ofs3 << fi << " "<< i << " " <<   F(fi,i)   << endl;
          ofs4 << fi << " "<< i << " " <<   Z(fi,i)   << endl;
          ofs5 << fi << " "<< i << " " << setscientific()<< setprecision(5) << relbio(fi,i)<< endl;
          ofs6 << fi << " "<< i << " " << setscientific()<< setprecision(5) << sum(N(fi,i))<< endl;
      }
      //ofs95 << fi << " " << setscientific()<< setprecision(5) << N(fi,1)     << endl;
      //ofs96 << fi << " " << setscientific()<< setprecision(5) << N(fi,nyrs)  << endl;
  }
  ...
  for (fi=1;fi<=num_fish;fi++) {
    ...
    //ofs14 << "Seldlt:" << fi << "   " << selplt(fi) << endl;
    //ofs14 << "Seldl:" << fi << "   " << selpl(fi) << endl;
  }

FUNCTION write_simulated_data_for_SS3_multi
  int i,j,fi,fj,fk;
  // ignoring spatial structure ,  OS add "j"   
  int nregions=num_fish;  // OS modified
  // count number of fishing situations
  int num_data_sets=0;
  double cutoff=1.0;
  for (fi=1;fi<=num_fish;fi++) {
  ...
  ofstream ofs("tunasim_out_forSS3multi.dat");
  ofs << "#V3.24f" << endl;
  ...
  for (fi=1;fi<=num_fish;fi++) {
    if(fi < num_fish) {
       ...
       ofs << fi << "   # area_assignments_for_each_fishery_and_survey" << endl;
       }
    }
  for (fi=1;fi<=num_fish;fi++) {
    if(fi < num_fish) {
  ...
  for (fi=1;fi<=num_fish;fi++) {
    for (i=1;i<=nyrs;i++) {
      ...
      if (sumcatch2>cutoff) {
         ofs << 1959+i <<  "   1   " << fi+num_fish << "   " << obs_catch(fi,i)/obs_effort(fi,i)/100000 << "   0.04" << endl;
         }
      }
    }
  //
  ...
  for (fk=1;fk<=nlint;fk++) {
    if(fk<nlint){
      ofs << " " << l1+(delta*(fk-1));
      }
    else {
      ofs << " " << l1+(delta*(fk-1)) << endl;
      }
    }
  ...
  //
  //
  ofstream ofs1("tunasim_out_forSS3multi.ctl");
  ofs1 << "#V3.24f" << endl;
  ...
  if (nregions>1) {
    ofs1 << "#" << endl;
    ofs1 << "# recruitment for each area #" << endl;
    if(spawn_area_sw==0) {
      ofs1 << nregions << "    # number of recruitment assignment (overrides growth pattern* area* seasons parameters)" << endl; 
      ofs1 << "0    # recruitment interaction requested" << endl;
      for (fi=1;fi<=nregions;fi++)
         ofs1 << "1 1 " << fi << " # assignment(growth pattern, season, area) " <<  endl;
    }
    if(spawn_area_sw==1) {
      ofs1 << "1    # number of recruitment assignment (overrides growth pattern* area* seasons parameters)" << endl; 
      ofs1 << "0    # recruitment interaction requested" << endl;
      ofs1 << "1 1 " << 1 << "   # assignment(growth pattern, season, area) " <<  endl;      
    }
    ofs1 << "#" << endl;
    ofs1 << "# movement #" << endl;
    ofs1 << num_move << "    # number of movement definitions" << endl;
    ofs1 << "0    # first age that moves (real age at begin of season, not integer)" << endl;
    ofs1 << "# seas, GP, source_area, dest_area, minage, maxage" << endl;
    for (fi=1;fi<=num_fish;fi++) {
      for (fj=1;fj<=nregions;fj++) {
         ofs1 << "1 1 " << fi << " " << fj << " 0 " << nages-1 << "  # movement from " << fi << " to " << fj << endl;   
      }
    }
  }
  ofs1 << "#" << endl;
  ...
  ofs1 << "#" << endl;
  ofs1 << "  0.00   0.00   0.00    0.00     -1    0      -4    0     0     0      0      0       0      0   # [MGparam[13]] recruitment apportionment (GP)." << endl;
  for (fi=1;fi<=nregions;fi++) {
      ofs1 << "  0.00   0.00   0.00    0.00     -1    0      -4    0     0     0      0      0       0      0   # [MGparam[" << fi+13 << "]] recruitment apportionment (Area " << fi << ")." << endl;
  }
  ofs1 << "  0.00   0.00   0.00    0.00     -1    0      -4    0     0     0      0      0       0      0   # [MGparam[" << nregions+14 << "]] recruitment apportionment (Season)." << endl;
  ofs1 << "  0.00   0.00   0.00    0.00     -1    0      -4    0     0     0      0      0       0      0   # [MGparam[" << nregions+15 << "]] cohort growth  dev. Dont omit." << endl;
  if (nregions>1) {
    for (fi=1;fi<=nregions;fi++) {
      for (fj=1;fj<=nregions;fj++) {
         if(fj==1) {
           ofs1 << "  -200    200   0.00    0.00     -1    99     -1    0     0     0      0      0.5      0      0   # [MGparam[*]] movement from " << fi << " to " << fj << " (minage)." << endl;
           ofs1 << "  -200    200   0.00    0.00     -1    99     -1    0     0     0      0      0.5      0      0   # [MGparam[*]] movement from " << fi << " to " << fj << " (maxage)." << endl;
         }
         else {
           ofs1 << "  -200    200   0.00    0.00     -1    99      3    0     0     0      0      0.5      0      0   # [MGparam[*]] movement from " << fi << " to " << fj << " (minage)." << endl;
           ofs1 << "  -200    200   0.00    0.00     -1    99      3    0     0     0      0      0.5      0      0   # [MGparam[*]] movement from " << fi << " to " << fj << " (maxage)." << endl;
         }
      }
    }
  }
  ...
  ofs1 << "0.02    #_F ballpark for tuning early phases" << endl;
  ...
  ofs1 << "#" << endl;
  ofs1 << "0   # number of changes to make to default Lambdas (default value is 1.0)" << endl;
...
FUNCTION write_simulated_data_for_SS3_mono
  int i,j,fi,fj,fk;
  // ignoring spatial structure ,  OS add "j"   
  int nregions=1;  
  // count number of fishing situations
  ...
  ofstream ofs("tunasim_out_forSS3mono.dat");
  ofs << "#V3.24f" << endl;
  ofs << "#Simulated data for SS3(ver3.24) analysis" << endl;
  ofs << "#" << endl;
  ofs << "###### Model Dimensions ##################" << endl;
  ...
  for (fi=1;fi<=num_fish;fi++) {
    if(fi < num_fish) {
       ...
       ofs << 1 << "   # area_assignments_for_each_fishery_and_survey (=1)" << endl;
       }
    }
  for (fi=1;fi<=num_fish;fi++) {
    if(fi < num_fish) {
  ...
  //
  //
  ofstream ofs1("tunasim_out_forSS3mono.ctl");
  ofs1 << "#V3.24f" << endl;
  ofs1 << "#Simulated data for SS3(ver3.24) analysis" << endl;
  // 
  ofs1 << "#" << endl;
  ...
  ofs1 << "#" << endl;
  ofs1 << "# recruitment for each area #" << endl;
  ofs1 << "#    # number of recruitment assignment (overrides growth pattern* area* seasons parameters)" << endl; 
  ofs1 << "#    # recruitment interaction requested" << endl;
  ofs1 << "#    # assignment(growth pattern, season, area) " <<  endl;
  ofs1 << "#" << endl;
  ofs1 << "# movement #" << endl;
  ofs1 << "#    # number of movement definitions" << endl;
  ofs1 << "#    # first age that moves (real age at begin of season, not integer)" << endl;
  ofs1 << "# seas, GP, source_area, dest_area, minage, maxage" << endl;
  ofs1 << "#    # movement from "  << endl;   
  ofs1 << "#" << endl;
  ofs1 << "0    # number of block patterns" << endl;
  // 
  ofs1 << "#" << endl;
  ...
  ofs1 << "#" << endl;
  ofs1 << "  0.00   0.00   0.00    0.00     -1    0      -4    0     0     0      0      0       0      0   # [MGparam[13]] recruitment apportionment (GP)." << endl;
  ofs1 << "  0.00   0.00   0.00    0.00     -1    0      -4    0     0     0      0      0       0      0   # [MGparam[14]] recruitment apportionment (Area " << fi << ")." << endl;
  ofs1 << "  0.00   0.00   0.00    0.00     -1    0      -4    0     0     0      0      0       0      0   # [MGparam[15]] recruitment apportionment (Season)." << endl;
